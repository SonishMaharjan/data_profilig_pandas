...
patients = ['patient_ssn', 'patient_firstName', 'patient_lastName', 'patient_country',
            ...
            'patient_sex','patient_DOB', 'patient_DOD', 'patient_email', 'patient_height', 'patient_weight',
...
for column in selected_columns:
    ...
    date_data_count_mmddyyyy = non_null_data.apply(lambda x: pd.to_datetime(x, format='%m/%d/%Y', errors='coerce')).notna().sum()
    total_non_null = len(non_null_data)

    date_percentage_mmddyyyy = round((date_data_count_mmddyyyy / total_non_null) * 100, 3) if total_non_null > 0 else 0

    data_percentage[column] = date_percentage_mmddyyyy
