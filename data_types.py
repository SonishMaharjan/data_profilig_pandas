...
for column in selected_columns:
    ...
    number_data_count = non_null_data.apply(lambda x: pd.to_numeric(x, errors='coerce')).notna().sum()
    total_non_null = len(non_null_data)

    number_percentage = round((number_data_count / total_non_null) * 100, 3) if total_non_null > 0 else 0
    string_percentage = round(100 - number_percentage, 3) if total_non_null > 0 else 0

    data_percentage[column] = {
        'string': string_percentage,
        'number': number_percentage
    }
